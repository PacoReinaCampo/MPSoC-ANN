  ...
  task compare_trans();
    ...
    if (exp_trans_fifo.size != 0) begin
      ...
      if (act_trans_fifo.size != 0) begin
        ...
        `uvm_info(get_full_name(), $sformatf("expected peripheral_adder SUM =%d , actual adder_8_bit SUM =%d ", exp_transaction.data_out, act_transaction.data_out), UVM_LOW);
        if (exp_transaction.data_out == act_transaction.data_out) begin
