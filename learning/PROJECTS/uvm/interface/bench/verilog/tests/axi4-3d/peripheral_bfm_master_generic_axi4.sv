...
module peripheral_bfm_master_generic_axi4 (
  ...
  output reg  [31:0] awadr_i,  // Write Address I
  output reg  [31:0] awadr_j,  // Write Address J
  output reg  [31:0] awadr_k,  // Write Address K
  ...
  output reg  [31:0] araddr_i,  // Read Address I
  output reg  [31:0] araddr_j,  // Read Address J
  output reg  [31:0] araddr_k,  // Read Address K
  ...
  task write_single;
    ...
    begin
      ...
      $display("TASK: Write Single Addr = 0x%4x Data = 0x%4x Size = 0x%x Strobe = 0x%x Time = %d", address_i, address_j, address_k, data, size, strobe, $time);
  ...
  task read_single;
    ...
    begin
      ...
      $display("TASK: Read Single Addr Addr = 0x%4x Data = 0x%4x Size = 0x%x Strobe = 0x%x Time = %d", address_i, address_j, address_k, data, size, strobe, $time);
    end
  endtask
  ...
  task read_single_and_check;
    ...
    reg [31:0] read_data;
    begin
      read_single(address_i, address_j, address_k, read_data, size, strobe);
