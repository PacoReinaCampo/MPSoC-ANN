interface lisnoclink_if (
  ...
  task send(input logic [flit_width-1:0] sflit, input integer vc);
    flit      = sflit;
  ...
  task receive(output logic [flit_width-1:0] rflit, input logic [vchannels-1:0] vcready = {vchannels{1'b1}});
    ready = vcready;
