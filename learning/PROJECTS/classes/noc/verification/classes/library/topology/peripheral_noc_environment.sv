...
class environment #(
  ...
  trafficgen #(.flit_data_width(flit_data_width), .flit_dest_width(flit_dest_width), .vchannels(vchannels), .mc_supported(mc_supported), .mc_num_dest(mc_num_dest)) traffic [] [];  // index 0: node, index 1: traffic gen at node

  sink #(flit_data_width, flit_dest_width, vchannels, mc_supported, mc_num_dest)                                                                                    sinks   [];

  networkacc #(.flit_data_width(flit_data_width), .flit_dest_width(flit_dest_width), .vchannels(vchannels), .mc_supported(mc_supported), .mc_num_dest(mc_num_dest)) netacc  [];

  measurement #(flit_data_width, flit_dest_width, mc_supported, mc_num_dest)                                                                                        measure;
  sysconfig                                                                                                                                                         conf;

  function new(
  input virtual lisnoclink_if #(
    .vchannels (vchannels),
    .flit_width(flit_data_width + 2 + mc_supported)
  ) links_out[],
  input virtual lisnoclink_if #(
    .vchannels (vchannels),
    .flit_width(flit_data_width + 2 + mc_supported)
